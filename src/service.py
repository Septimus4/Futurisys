...
from . import hf
from .models import InferenceRequest, InferenceResult, InferenceError
...
def classify_and_persist(payload: ClassifyRequest, db: Session, api_key_hash: str | None) -> ClassifyResponse:
    ...
    except FuturesTimeout as e:  # timeout handling
        trace = tb.format_exc()
        inf_err = InferenceError(
            request_id=request_id,
            error_type="TimeoutError",
            message=f"inference exceeded {settings.inference_timeout_seconds}s",
            traceback=trace,
        )
        db.add(inf_err)
        db.commit()
    ...
    labels: list[str] = list(result["labels"])  # type: ignore[index]
    scores: list[float] = list(result["scores"])  # type: ignore[index]
